...
const marqueeplace = document.getElementById('mymarquee');
async function gettingMarquee(){

  const url = `https://stock-exchange-dot-full-stack-course-services.ew.r.appspot.com//api/v3/quotes/nyse`
  const response = await fetch(url);
  const result = await response.json();
  console.log('marquee',result);
  for(let i=0; i<500; i++ ){
    const object = result[i];
    printingOnMarquee(marqueeplace,object)
...
function printingOnMarquee(container,dataobject){
  marqueeplace.innerHTML += `<span>${dataobject.symbol}</span>`+ `<span class ="pricecolor"> ${dataobject.price}</span>` + `   `
